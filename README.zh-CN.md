...
- **字体处理**: [opentype.js](https://opentype.js.org/) - 专业的字体解析和操作库（纯 JavaScript）
- **性能优化**: Web Workers - 后台处理，避免阻塞主线程
...
├── style.css               # 样式文件
├── app.js                  # 主要逻辑（Web 版本）
├── font-worker.js          # Web Worker 字体处理
├── i18n.js                 # 国际化支持
├── package.json            # Node.js 依赖配置（用于 Web 版本）
├── .github/
│   ├── workflows/
│   │   └── font-subset.yml # GitHub Actions 工作流
│   └── scripts/
│       ├── process-font.py # 字体处理脚本（Python）
...
**GitHub Actions 版本**使用 Python 和 pip 管理依赖：

- [fonttools](https://github.com/fonttools/fonttools) - 专业的字体处理库 (pyftsubset)
- Python 内置库用于 ZIP 文件生成
...
> **注意**: GitHub Actions 工作流现在使用 Python 而不是 Node.js，以便在处理大字体文件时获得更好的内存效率。Web 版本仍然使用 opentype.js 以保持浏览器兼容性。
